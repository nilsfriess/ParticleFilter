...
private:
  ...
  const arma::dmat m_sys_cov{{0.1, 0}, {0, 0.1}};
...
public:
  ...
  virtual void sample_prior(smcpf::Particle<arma::dvec> &t_particle) const override {
  ...
  double update_weight(const smcpf::Particle<arma::dvec> &t_curr_particle,
                       const double &t_observation,
                       [[maybe_unused]]double t_time) const override {
    const arma::dvec mu = m_obs_mat * evolve(t_curr_particle, t_time);
